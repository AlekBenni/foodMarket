...
import { addProductToSweetReducerAC } from '../../Redux/SweetReducer'
import { NavLink } from 'react-router-dom'

function Sweet (){

    let sweetCount = useSelector((state:AppStateType) => state.blockSweet)

    let resultSweetCount = sweetCount.map(item => {
        return (
    ...
    let addSweetProduct = () => {  
      dispatchSweetProduct(addProductToSweetReducerAC(price, title, images, availability,
      description, consist
      ))
    }
    ...
    return (
        ...
        <div className="alert alert-info" role="alert">
            <h5>Все товары категории конфеты:</h5>
        </div>
