...
import { addProductToSendwichReducerAC } from '../../Redux/SendwichReducer'
import { NavLink } from 'react-router-dom'

function Sendwich (){

    let sendwichCount = useSelector((state:AppStateType) => state.blockSendwich)

    let resultsendwichCount = sendwichCount.map(item => {
        return (
    ...
    let dispatchSweetProduct = useDispatch()
    ...
    let addSweetProduct = () => {  
      dispatchSweetProduct(addProductToSendwichReducerAC(price, title, images, availability,
      description, consist
      ))
    }
    ...
    return (
        ...
        <div className="alert alert-info" role="alert">
            <h5>Все товары категории сэндвич:</h5>
        </div>
